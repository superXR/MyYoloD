...
if __name__ == "__main__":
    ...
    for k in state_dict.keys():
        index_str = k.split('.')[1]
        v = state_dict[k]
        if int(index_str) >= 25 and int(index_str) <= 33:
            continue
        elif int(index_str) >= 34:
            k = k.replace(index_str, str(int(index_str) - 9))
        new_state_dict[k] = v
    ...
    try:
        ...
        print('read onnx using onnxruntime success')
